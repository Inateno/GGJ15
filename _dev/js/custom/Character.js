...
function( data, DE )
{
  ...
  function Character( x, y, tag, colliderW, colliderH, sprite )
  {
    ...
    this.makeCollision = function()
    {
      if ( !this.locked )
      {
        for ( var i = 0; i < this.scene.event_triggers.length; ++i )
        {
          if ( DE.CollisionSystem.fixedBoxCollision( this.scene.event_triggers[ i ].collider, this.collider ) )
          {
            switch( this.scene.event_triggers[ i ].name )
            {
              case "goal":
                this.onGoal = true;
                DE.trigger( "player" + this.playerId + "TouchEnd" );
                break;
            }
          }
        }
    ...
    this.bindControls = function( playerId )
    {
      this.playerId = playerId;
      DE.Inputs.on( "axeMoved", "haxe" + playerId, function(val){ _self.updateAxes( val, undefined ); } );
      DE.Inputs.on( "axeMoved", "vaxe" + playerId, function(val){ _self.updateAxes( undefined, val ); } );
      DE.Inputs.on( "axeStop", "haxe" + playerId, function(){ _self.updateAxes( 0, undefined ); } );
      DE.Inputs.on( "axeStop", "vaxe" + playerId, function(){ _self.updateAxes( undefined, 0 ); } );
      DE.Inputs.on( "keyUp", "left" + playerId, function(){
        if ( DE.Inputs.key( 'right' + playerId ) )
          return;
        _self.updateAxes( 0, undefined );
      } );
      DE.Inputs.on( "keyUp", "right" + playerId, function(){
        if ( DE.Inputs.key( 'left' + playerId ) )
          return;
        _self.updateAxes( 0, undefined );
      } );
      DE.Inputs.on( "keyDown", "left" + playerId, function(){ _self.updateAxes( -1, undefined ); } );
      DE.Inputs.on( "keyDown", "right" + playerId, function(){ _self.updateAxes( 1, undefined ); } );
      DE.Inputs.on( "keyDown", "up" + playerId, function(){ _self.updateAxes( undefined, -1 ); } );
      DE.Inputs.on( "keyDown", "down" + playerId, function(){ _self.updateAxes( undefined, 1 ); } );
      DE.Inputs.on( "keyUp", "up" + playerId, function(){ _self.updateAxes( undefined, 0 ); } );
      DE.Inputs.on( "keyUp", "down" + playerId, function(){ _self.updateAxes( undefined, 0 ); } );
      DE.Inputs.on( "keyDown", "jump" + playerId, function(){ _self.jump(); } );
      DE.Inputs.on( "keyUp", "jump" + playerId, function(){ _self.resetJump(); } );
